...
function setForm(){
    ...
    $("#pasdetempsS1").on("change", function(){
        ...
        $("#variableS1").on("change", function(){
            ...
            if (($("#produitS1").val() == "omaeruv") && (varInfos.variables.dims[id].first() != -1)){
    ...
    if($('#checkboxSr2').is(':checked')){
        ...
        $("#pasdetempsS2").on("change", function(){
            ...
            $("#variableS2").on("change", function(){
                var id = $(this).prop('selectedIndex');
                if ($("#produitS2").val() == "omaeruv"){
                    $("#levelSr2").prop("disabled", false);
                    $.each(varInfos.variables.dims[id], function (i, item) {
                        $('#levelSr2').append($('<option>', {value: item, text : item}));
                    });
                }
            });
        ...
        $('.input-small').keypress(function(event) {
            if(event.which == 8 || event.keyCode == 9 || event.keyCode == 37 || event.keyCode == 39 || event.keyCode == 45 || event.keyCode == 46) 
                return true;
            else if((event.which != 45 || event.which != 46) && (event.which < 48 || event.which > 57))
