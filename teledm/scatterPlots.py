...
def readNC_box(nc_file, variable, xhg, yhg, xbd, ybd, date1, date2,prd_sat, lev, frequence, h_passage=13,station_lon=[], station_lat=[], buff=[]):
    ...
    try:
        idj1 = date2index(date1, temps, select='exact')#indice de la date de debut
    except ValueError:
        return 0,0,0
    try:
        idj2 = date2index(date2, temps, select='exact')#indice de la date de fin
    except ValueError:
        return 0,0,0
...
def heure_passage(station_aeronet):
    ...
    coord_st=pd.read_csv(os.path.join(ddirDB,'in_situ/aeronet/carto_aeronet/coord_aeronet.csv'), sep=',', header=0)
